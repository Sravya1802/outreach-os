...
Interactive mode for when the candidate is filling out an application form in Chrome. It reads what's on screen, loads the prior context for the offer, and generates personalized answers for each form question.
...
5. COMPARE    → Does the on-screen role match the one evaluated? If changed → alert
